...
let isCreator = false
class SoChungThuc extends React.Component {
	constructor(props) {
	  super(props);
	  this.state = {
			branchId: 0,
	...
	onMessage = async (msg) => {
		...
		if(msg.type == 'ATTESTATION_BOOK' && msg.action == 'ADD') {
		...
		} else if(msg.type == 'ATTESTATION_BOOK' && msg.action == 'UPDATE'){ //attestation
			...
			isCreator = false
		} else if(msg.type == 'ATTESTATION_BOOK' && msg.action == 'DELETE'){
	...
	onChangeActive = (e, index) => {
 	...
 	render() {
		 const { showMenu, branchId, branch, popupTitle, popupContent, customizeCancelButton, customizeOkButton, isShowPopup, indexEditting, showCancelButton, userInfo, isAdding, page } = this.state
		...
		<div style={{marginTop: 20, marginLeft: 20}}>
			...
			<div style = {{flexDirection: 'row', display: 'flex', justifyContent:'space-between'}}>
				<div style={{marginTop: 20}}>
					<button  style={{width: 150, height: 40, fontSize: 14, fontWeight: 'bold'}}  onClick = {this.toCongChung}>Công chứng</button>
					<button style={{width: 150, height: 40, backgroundColor:'#28a745', fontSize: 14, fontWeight: 'bold'}}>Chứng thực</button>
		  ...
		  <table className="table table-bordered">
			<thead>
			  <tr>
				<th>Stt</th>
				<th>Số chứng thực</th>
				<th>Cá nhân, tổ chức thực hiện chứng thực</th>
				<th>Người làm</th>
				<th>Lệ phí</th>
				<th>Thu lệ phí</th>
				<th>CCV xác nhận</th>
  ...
  class ProductRow extends React.PureComponent {
	render() {
		const {index, indexEditting, userInfo, product, isAdding} = this.props
	  return (
  ...
  class EditableCell extends React.PureComponent {
	...
	render() {
		...
		let type ='text'
		if(cellData.type === 'fee' || cellData.type === 'paidAmount') type = 'number'
	  return (
		<td>
			<input 
...
export default connect(mapStateToProps, null)(SoChungThuc);